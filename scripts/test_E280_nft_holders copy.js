...
const CACHE_TTL = 1000 * 60 * 60; // 1 hour TTL for in-memory cache
let inMemoryCache = {};
...
// Fetch burned tokens via Transfer events
async function fetchBurnedTokens(context, client, contractAddress, startBlock, endBlock, batchSize = 10000) {
  const cacheKey = `${contractAddress}-burns`;
  const now = Date.now();
  if (inMemoryCache[cacheKey] && now - inMemoryCache[cacheKey].timestamp < CACHE_TTL) {
    logger.info(context, `Returning cached burn data for ${cacheKey}`);
    return inMemoryCache[cacheKey].data;
  }

  logger.info(context, `Fetching burned tokens from block ${startBlock} to ${endBlock}`);
  let burns = [];
  ...
  for (let fromBlock = startBlock; fromBlock <= endBlockNumber; fromBlock += batchSize) {
    ...
    try {
      ...
      const burnLogs = logs.filter((log) => log.args.to.toLowerCase() === burnAddress.toLowerCase());
      burns = burns.concat(burnLogs);
      logger.debug(context, `Fetched ${burnLogs.length} burns from ${fromBlock} to ${toBlock} (total: ${burns.length})`);
    } catch (error) {
      logger.warn(context, `Failed to fetch burns for blocks ${fromBlock}-${toBlock}: ${error.message}`);
      if (error.message.includes('Log response size exceeded')) {
        logger.warn(context, `Retrying with smaller block range for ${fromBlock}-${toBlock}`);
        const smallerBatchSize = Math.floor(batchSize / 2);
        if (smallerBatchSize < 1000) {
          logger.error(context, `Block range too small to proceed: ${smallerBatchSize}`);
          continue;
        }
        const subBurns = await fetchBurnedTokens(
          context,
          client,
          contractAddress,
          fromBlock,
        ...
        burns = burns.concat(subBurns);
        fromBlock += smallerBatchSize - batchSize; // Adjust for loop increment
      }
    }
  }

  inMemoryCache[cacheKey] = { timestamp: now, data: burns };
  const cache = await loadCache();
  cache[cacheKey] = { timestamp: now, burns: burns.length };
...
async function fetchOwnersAlchemy(contractAddress, contractKey) {
  ...
  try {
    ...
    logger.error(context, `Failed to fetch owners via Alchemy: ${error.message}`, { stack: error.stack });
    throw error;
  }
}
...
// NFT contract configuration for Element 280
const nftContracts = {
  element280: {
    name: 'Element 280',
    symbol: 'ELMNT',
...
async function testNFTHolders() {
  ...
  try {
    // Step 1: Fetch totalSupply, totalBurned, and burned tokens
    ...
    // Fetch burned tokens via Transfer events
    const burnLogs = await fetchBurnedTokens(
      context,
      client,
      contractAddress,
    ...
    summary.burnedTokens = burnLogs.length;
    logger.info(context, `Verified burned tokens via events: ${summary.burnedTokens}`);

    // Validate burned tokens
    if (summary.burnedTokens !== summary.totalBurned) {
      summary.mismatch = `Burned tokens mismatch: ${summary.burnedTokens} (events) vs ${summary.totalBurned} (contract)`;
      logger.warn(context, summary.mismatch);
    }

    // Validate totalLiveSupply
    const expectedLiveSupply = 8079; // From test logs
    if (totalTokens !== expectedLiveSupply) {
      ...
      logger.warn(context, `Live supply mismatch: ${totalTokens} vs expected ${expectedLiveSupply}`);
    }

    summary.timings.totalSupply = Number(process.hrtime.bigint() - stepStart) / 1_000_000;

    // Step 2: Fetch owners using Alchemy SDK
    ...
    if (tokenIds.length > 0) {
      ...
      if (uncachedTokenIds.length > 0) {
        ...
        const chunkSize = 200; // Reduced from 500
        ...
        for (let i = 0; i < tierCalls.length; i += chunkSize * concurrencyLimit) {
          ...
          logger.info(
            context,
            `Tiers progress: ${Math.min(((i + chunkSize * concurrencyLimit) / tierCalls.length) * 100).toFixed(2)}%`
          );
        }
      ...
      for (let i = 0; i < holders.length; i += batchSize) {
        ...
        batchResults.forEach(({ holder, result, error }) => {
          if (error) {
            ...
            logger.warn(context, `Failed to fetch rewards for ${holder.wallet}: ${error}`);
            ...
            logger.debug(context, `Rewards for ${holder.wallet}: ${holder.claimableRewards}`);
          } else {
            holder.claimableRewards = 0;
            logger.warn(context, `Invalid reward result for ${holder.wallet}: ${result?.error || 'Unknown error'}`);
          }
        });

        if (i + batchSize < holders.length) {
          logger.debug(context, `Waiting 500ms before next reward batch`);
          await new Promise((resolve) => setTimeout(resolve, 500));
        }
      }
    } else {
      logger.warn(context, `Vault address not set or invalid, skipping rewards`);
    }
...
async function main() {
  try {
    ...
    if (summary.error) {
      ...
      logger.warn('test', 'Test completed with mismatches');
      process.exit(0);
    } else {
