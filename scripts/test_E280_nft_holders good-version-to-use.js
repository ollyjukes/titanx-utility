...
const providers = [
  {
    ...
    rateLimit: 10000, // Adjust based on paid tier
    requestCount: 0,
    lastRequestTime: 0,
  },
];
...
const clients = [
  {
    client: createPublicClient({
      chain: mainnet,
      transport: http(providers[0].url, { timeout: 60000 }), // 60s timeout
    }),
    providerIndex: 0,
  },
];
...
async function fetchDispositionEvents(context, client, contractAddress, startBlock, endBlock, batchSize = 10000) {
  ...
  const concurrencyLimit = 8; // Increased for paid tier
...
async function testNFTHolders() {
  ...
  try {
    // Step 1: Fetch totalSupply, totalBurned, and vault totals
    ...
    const [nftResults, vaultResults] = await Promise.all([
      retry(() =>
        client.multicall({
          contracts: nftCalls,
    ...
    const totalE280Burned = vaultResults[0].status === 'success' ? vaultResults[0].result : 0n;
    const totalRewardsPaid = vaultResults[1].status === 'success' ? vaultResults[1].result : 0n;
    const totalRewardPool = vaultResults[2].status === 'success' ? vaultResults[2].result : 0n;
    ...
    summary.totalE280Burned = Number(totalE280Burned) / 1e18;
    summary.totalRewardsPaid = Number(totalRewardsPaid) / 1e18;
    summary.totalRewardPool = Number(totalRewardPool) / 1e18;
    logger.info(
      `[${context}] State: live=${totalTokens}, burned=${summary.totalBurned}, minted=${summary.totalMinted}, e280Burned=${summary.totalE280Burned.toFixed(2)}, rewardsPaid=${summary.totalRewardsPaid.toFixed(2)}, rewardPool=${summary.totalRewardPool.toFixed(2)}`
    );
    ...
    for (let i = 0; i < filteredOwners.length; i++) {
      ...
      if (i % 100 === 0 || i === filteredOwners.length - 1) {
    ...
    if (tokenIds.length > 0) {
      ...
      if (uncachedTokenIds.length > 0) {
        ...
        const chunkSize = 300; // Increased for performance
        ...
        for (let i = 0; i < tierCalls.length; i += chunkSize * concurrencyLimit) {
          ...
          const processed = Math.min(i + chunkSize * concurrencyLimit, tierCalls.length);
    ...
    if (holders.length > 0 && vaultAddresses.element280 && isAddress(vaultAddresses.element280)) {
      ...
      const batchSize = 150; // Increased for performance
    ...
    if (process.env.LOG_WALLETS === 'true') {
      holders.forEach((holder) => {
        logger.info(
          `[${context}] Wallet: ${holder.wallet}, Tokens: ${holder.total}, Tiers: ${holder.tiers}, MultiplierSum: ${holder.multiplierSum}, Rank: ${holder.rank}, Percentage: ${holder.percentage.toFixed(2)}%, Rewards: ${holder.claimableRewards}`
        );
      });
    }
...
async function main() {
  try {
    ...
    console.table({
      ...
      TotalE280Burned: summary.totalE280Burned.toFixed(2),
      TotalRewardsPaid: summary.totalRewardsPaid.toFixed(2),
      TotalRewardPool: summary.totalRewardPool.toFixed(2),
