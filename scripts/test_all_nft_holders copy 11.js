...
const clients = [{
  client: createPublicClient({
    chain: mainnet,
    transport: http(providers[0].url, { timeout: 10000 }), // 10s timeout
  }),
  providerIndex: 0,
}];
...
async function fetchOwnersOnChain(contractAddress, totalSupply, abi, contractKey, client) {
  ...
  const chunkSize = 100; // Increased for fewer requests
  ...
  try {
    ...
    const concurrencyLimit = 2; // Parallelize for all collections
    for (let i = 0; i < chunks.length; i += concurrencyLimit) {
        ...
        chunkBatch.map(async ({ start, end, tokenIds }, idx) => {
          ...
          const results = await retry(async () => {
            const multicallResult = await client.multicall({
              ...
              allowFailure: true,
              blockNumber: null, // Fetch latest block
            });
            // Get the block number of the multicall
          ...
          results.forEach(({ tokenId, owner }) => {
            if (owner && isAddress(owner)) {
              ...
              contractCache.nonExistent.push(tokenId); // Cache non-existent
            }
          });
          ...
          if (idx < chunkBatch.length - 1) await new Promise(resolve => setTimeout(resolve, 200)); // Stagger concurrent requests
          return results;
        })
      );
      ...
      if ((i + concurrencyLimit) % 10 === 0 || i + concurrencyLimit >= chunks.length) {
        await saveCache(cache); // Save every 10 chunks or at end
      }
    ...
    await saveCache(cache); // Final save
...
async function testNFTHolders(contractKey) {
  ...
  let summary = {
    ...
    tierCounts: {}, // To store tier counts
    status: 'Failed',
    mismatch: null,
    timings: {
      totalSupply: 0,
  ...
  try {
    ...
    if (contractKey === 'ascendant') {
      ...
      summary.totalLiveSupply = totalMinted; // Set totalLiveSupply for Ascendant
      summary.totalMinted = 'N/A'; // Set totalMinted to N/A
    ...
    if (tokenIds.length > 0) {
      ...
      if (uncachedTokenIds.length > 0) {
        ...
        const chunkSize = 100; // Increased for tiers
      ...
      const tierTable = Object.entries(summary.tierCounts).map(([tier, data]) => {
        if (summary.contractKey === 'ascendant') {
          ...
          const raritySummary = Object.entries(rarityCounts)
            .map(([rarity, count]) => `Rarity ${rarity}: ${count}`)
            .join(', ');
          return {
            Tier: tier,
            Name: data.name,
