...
async function fetchOwnersOnChain(contractAddress, totalSupply, abi, contractKey) {
  ...
  try {
    ...
    logger.info(
      context,
      `Fetched ${owners.length} owners on-chain, ${burnedTokens} burned tokens, ${nonExistentTokens} non-existent tokens`,
      'eth',
      'nft'
    );
    return owners;
...
async function testNFTHolders(contractKey) {
  ...
  try {
    ...
    const owners = await fetchOwnersOnChain(contractAddress, totalTokens, abi, contractKey);
    ...
    if (contractKey !== 'ascendant') {
      ...
      if (tokenCount < expectedLiveTokens) {
        logger.warn(
          context,
          `Mismatch: Processed ${tokenCount} tokens, but totalLiveSupply=${expectedLiveTokens}. Missing ${missingTokens} tokens (likely ${burnedTokens} burned, ${nonExistentTokens} non-existent).`,
          'eth',
          'nft'
        );
