...
async function fetchOwnersOnChain(contractAddress, totalSupply, abi, contractKey) {
  ...
  try {
    ...
    // Log raw owners summary (avoid verbose list for large collections)
    if (process.env.DEBUG_NON_EXISTENT === 'true') {
      logger.info(
        context,
...
async function testNFTHolders(contractKey) {
  ...
  try {
    ...
    const invalidOwners = owners.filter(
      owner => !owner?.ownerAddress || owner.tokenBalances?.length === 0
    );
...
// In the tier-fetching section (Step 4 of testNFTHolders)
tierResults.forEach(result => {
  if (result.status === 'success' && result.result !== null) {
    ...
    if (contractKey === 'ascendant') {
      ...
      // Expect result to be a tuple: [rarityNumber, tier, rarity]
      if (Array.isArray(result.result) && result.result.length === 3) {
        tier = Number(result.result[1]); // tier is at index 1
        if (tier === 0) {
          logger.warn(context, `Tier is 0 for token ${tokenId}, full result: ${JSON.stringify(result.result)}`, 'eth', 'nft');
        } else if (tier < 1 || tier > 8) {
          logger.warn(context, `Unexpected tier value ${tier} for token ${tokenId}, full result: ${JSON.stringify(result.result)}`, 'eth', 'nft');
        }
      } else {
    ...
    if (contractKey !== 'ascendant') {
      ...
      if (tokenCount < expectedLiveTokens) {
        summary.mismatch = `Processed ${tokenCount} tokens, but totalLiveSupply=${expectedLiveTokens}. Missing ${missingTokens} tokens (likely ${burnedTokens} burned, ${nonExistentTokens} non-existent). Non-existent token IDs: [${nonExistentTokenIds.join(', ')}]`;
      ...
      summary.status = 'Success'; // Ascendant has no totalSupply validation
    }
