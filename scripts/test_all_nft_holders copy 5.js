...
const providers = [
  ...
  // Add more providers if available (e.g., Infura, Ankr)
      ...
      tierResults.forEach(result => {
        if (result.status === 'success' && result.result !== null) {
          ...
          if (wallet && tier > 0 && tier <= maxTier) {
            ...
            logger.warn(context, `Invalid or out-of-range tier (${tier}) for token ${tokenId}, wallet: ${wallet || 'none'}`, 'eth', 'nft');
          }
        } else {
