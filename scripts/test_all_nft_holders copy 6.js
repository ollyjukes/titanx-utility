...
async function fetchOwnersOnChain(contractAddress, totalSupply, abi, contractKey) {
  ...
  const chunkSize = 50; // Consider increasing to 100 for faster execution
  const context = `test/${contractKey}`;
  let burnedTokens = 0;
  ...
  try {
    ...
    for (let start = 1; start <= totalSupply; start += chunkSize) {
      ...
      const results = await retry(
        ...
        { retries: 5, delay: 2000, backoff: true } // Consider reducing retries to 3 or increasing delay to 3000
      );
...
async function testNFTHolders(contractKey) {
  ...
  // Define totalStart before the try block to ensure it's accessible in catch
    ...
    if (tokenIds.length > 0) {
      ...
      if (uncachedTokenIds.length > 0) {
        ...
        for (let i = 0; i < tierCalls.length; i += chunkSize) {
          ...
          // Rotate clients to distribute load
          const currentClient = clients[Math.floor(i / chunkSize) % clients.length];
          ...
          try {
            const results = await retry(
              ...
              { retries: 5, delay: 1000, backoff: true }
            );
        ...
        tierResults.forEach(result => {
          if (result.status === 'success' && result.result !== null) {
            contractCache[result.tokenId] = result.result;
          }
        });
        ...
        if (result.status === 'success' && result.result !== null) {
          ...
          if (contractKey === 'ascendant') {
            if (process.env.DEBUG_TIERS === 'true') {
              logger.debug(context, `Raw getNFTAttribute result for token ${tokenId}: ${JSON.stringify(serializeForLogging(result.result))}`, 'eth', 'nft');
            }
            if (!Array.isArray(result.result) || result.result.length !== 3) {
              logger.error(context, `Invalid tuple format for token ${tokenId}: ${JSON.stringify(serializeForLogging(result.result))}`, 'eth', 'nft');
              return;
            }
            tier = Number(result.result[1]);
            if (tier === 0) {
              logger.warn(context, `Tier is 0 for token ${tokenId}, full result: ${JSON.stringify(serializeForLogging(result.result))}`, 'eth', 'nft');
              return; // Skip invalid tier
            }
            if (tier < 1 || tier > 8) {
              logger.warn(context, `Unexpected tier value ${tier} for token ${tokenId}, full result: ${JSON.stringify(serializeForLogging(result.result))}`, 'eth', 'nft');
              return; // Skip out-of-range tier
            }
          } else {
