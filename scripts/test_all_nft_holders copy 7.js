  ...
  try {
    ...
    for (const owner of filteredOwners) {
      ...
      try {
        ...
        logger.warn(context, `Invalid wallet address مرحله: ${owner.ownerAddress}`, 'eth', 'nft');
        continue;
      }
          ...
          if (contractKey === 'ascendant') {
            ...
            if (
              ...
              if (parsedResult.rarity === 0) {
                logger.warn(context, `Token ${tokenId} has rarity=0, treating as valid but noting for review`, 'eth', 'nft');
              }
            } else {
