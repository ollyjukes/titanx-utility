...
async function testNFTHolders(contractKey) {
  ...
  try {
    ...
    if (tokenIds.length > 0) {
      ...
      tierResults.forEach(result => {
        ...
        if (contractKey === 'ascendant') {
          ...
          // Validate values
          if (
            typeof parsedResult.rarityNumber !== 'bigint' || // Check if BigInt
            parsedResult.rarityNumber < 0n || // Ensure non-negative
    ...
    logger.info(context, `Summary: ${holdersMap.size} holders, ${tokenCount} tokens, live=${totalTokens}, minted=${totalMinted}`);
