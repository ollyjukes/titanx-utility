...
async function fetchOwnersOnChain(contractAddress, totalSupply, abi, contractKey) {
  ...
  try {
    ...
    logger.info(context, `Fetched ${owners.length} owners on-chain`, 'eth', 'nft');
    return owners;
  } catch (error) {
...
async function testNFTHolders(contractKey) {
  ...
  try {
    ...
    let owners;
    try {
      owners = await fetchOwnersWithNFTApi(contractAddress, contractKey, { withTokenBalances: true, maxPages: 100 });
    } catch (nftApiError) {
      logger.warn(context, `NFT API failed: ${nftApiError.message}. Falling back to on-chain ownerOf`, 'eth', 'nft');
      owners = await fetchOwnersOnChain(contractAddress, totalTokens, abi, contractKey);
    }
