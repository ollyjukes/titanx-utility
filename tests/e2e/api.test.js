...
const app = createServer(async (req, res) => {
  ...
  if (pathname === '/api/init') {
    const { GET } = await import('@/app/api new code/init/route');
    return GET(req);
  } else if (pathname.startsWith('/api/holders/')) {
    if (pathname.endsWith('/progress')) {
      const { GET } = await import('@/app/api new code/holders/[contract]/progress/route');
      return GET(req, { params: { contract: pathname.split('/')[3] } });
    } else if (pathname.includes('/validate-burned')) {
      const { POST } = await import('@/app/api new code/holders/Element280/validate-burned/route');
      return POST(req);
    } else {
      const { GET } = await import('@/app/api new code/holders/[contract]/route');
      return GET(req, { params: { contract: pathname.split('/')[3] } });
    }
